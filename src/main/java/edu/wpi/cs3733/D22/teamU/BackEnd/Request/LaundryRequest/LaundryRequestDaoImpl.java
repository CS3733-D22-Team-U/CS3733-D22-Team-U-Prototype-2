...
public class LaundryRequestDaoImpl implements DataDao<LaundryRequest> {
    ...
    public void CSVToJava() throws IOException {
        ...
        while ((s = br.readLine()) != null)
        {
            ...
            if (row.length == columns) {
                laundryRequestsList.add(new LaundryRequest(row[0], row[1], row[2], row[3], row[4], row[5], row[6], row[7]));
            }
        }
    }
    @Override
    ...
    public void SQLToJava() {
        ...
        try {
            ...
            while (results.next()) {
                ...
                LaundryRequest SQLRow = new LaundryRequest(ID, name, patientName, staff, status, location, date, time);
    ...
    public void printTable() throws IOException {
        CSVToJava();
        System.out.println("ID |\t Type |\t Patient Name |\t Staff |\t Status |\t Location |\t Date |\t Time");
    ...
    public LaundryRequest askUser() {
        ...
        return new LaundryRequest(inputID, inputName, inputPatient, inputStaff,inputStatus,inputLocation,inputDate,inputTime);
    }
}
