...
public class EquipmentDeliverySystemController implements Initializable, IService {
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		String csvEquipmentRequest = "src/main/resources/TowerEquipmentRequests.csv";
		...
		for (int i = 0; i < checkBoxesInput.size(); i++) {
			int finalI = i;
			checkBoxesInput.get(i).disableProperty().bind(Bindings.createBooleanBinding(() -> !checkBoxes.get(finalI).isSelected(), checkBoxes.stream().map(CheckBox::selectedProperty).toArray(Observable[]::new)));
		}
		clearButton.disableProperty().bind(Bindings.createBooleanBinding(() -> checkBoxes.stream().noneMatch(JFXCheckBox::isSelected), checkBoxes.stream().map(CheckBox::selectedProperty).toArray(Observable[]::new)));

		submitButton.disableProperty().bind(Bindings.createBooleanBinding(() -> checkBoxes.stream().noneMatch(JFXCheckBox::isSelected), checkBoxes.stream().map(CheckBox::selectedProperty).toArray(Observable[]::new)));
	}
	...
	private ObservableList<EquipmentUI> newRequest(String name, int amount, String date, String time) {
	...
	private ObservableList<EquipmentUI> getEquipmentList() {
		...
		for (Equipment equipment : udb.EquipmentImpl.EquipmentList) {
			equipmentUI.add(new EquipmentUI(equipment.getName(), equipment.getInUse(), equipment.getAvailable(), equipment.getAmount()));
		}
	...
	private ObservableList<EquipmentUI> getActiveRequestList() {
		for (RequestEquip request : udb.requestEquipImpl.requestEquipList) {
			equipmentUIRequests.add(new EquipmentUI(request.getName(), request.getAmount(), request.getDate(), request.getTime()));
		}
		return equipmentUIRequests;
	}
	...
	public void increase() throws IOException {
		try {
			...
			udb.EquipmentImpl.editEquipValue("src/main/resources/TowerEquipment.csv", "Masks", udb.EquipmentImpl.EquipmentList.get(1).getAmount() + 1, udb.EquipmentImpl.EquipmentList.get(1).getInUse() + 1);
	...
	public void submitRequest() {
		...
		for (JFXCheckBox checkBox : checkBoxes) {
			if (checkBox.isSelected()) {
				...
				activeRequestTable.setItems(newRequest(checkBox.getText(), requestAmount, sdf3.format(timestamp).substring(0, 10), sdf3.format(timestamp).substring(11)));
				try {
					udb.requestEquipImpl.addRequest("src/main/resources/TowerEquipmentRequests.csv", checkBox.getText(), Integer.parseInt(checkBoxesInput.get(checkBoxes.indexOf(checkBox)).getText()), sdf3.format(timestamp).substring(0, 10), sdf3.format(timestamp).substring(11));
				} catch (IOException e) {
					e.printStackTrace();
				}
			}
